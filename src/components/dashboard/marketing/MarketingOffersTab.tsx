...
export function MarketingOffersTab({ recommendations, onGenerateRecommendation, onStatusChange, isGenerating, quota, onOpenFeedback, recosUsage }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-xl font-bold text-foreground font-heading">Recommandations Offres & Bundles</h3>
          ...
          {isGenerating ? (
            ...
            <>
              <Button onClick={onGenerateRecommendation} disabled={quota.remaining <= 0} className="w-full" size="lg">
                ...
                Générer une recommandation Offres
      ...
      {recommendations.length === 0 && !isGenerating ? (
        <p className="text-sm text-muted-foreground py-8 text-center">
          Aucune recommandation Offres pour le moment.{!generationBlocked && " Cliquez sur le bouton ci-dessus pour en générer une."}
        </p>
      ) : (
